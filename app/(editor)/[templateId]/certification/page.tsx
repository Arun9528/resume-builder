...
import {CertificateData } from "@/types/type";
...
import {addCertificate, deleteCertificate, updateCertificate } from "@/store/slices/resumeSlice";
...
export default function Certification() {
  ...
  const Certificates = useSelector((state: RootState) => state.resumeBuilder.certificates);
  ...
  const onSubmit: SubmitHandler<CertificateData> = (data) => {
    if (isEdit) {
  ...
  const handleEdit = (id: string) => {
    const found = Certificates.find((d) => d.id === id);
    if (!found) return;
    reset(found);
    setIsEdit(true);
  };
  const onNext = async () => {
    ...
    if (EmptyDataChecked) {
      if (Certificates.length === 0) {
        const msg = "Please add at least one Certificate before proceeding.";
    ...
    <div>
      ...
      >
        ...
        <Inputs
          ...
          placeholder="Please Fill The Certificate Name"
          register={register}
          validation={{
      ...
      <AllSectionValue<CertificateData>
        title="Certificates"
        items={Certificates}
        ...
        renderItem = {(item)=> (
          <div>
                ...
                <p>{item.certificationcompanyname}</p>
           </div>
        )}
      />
