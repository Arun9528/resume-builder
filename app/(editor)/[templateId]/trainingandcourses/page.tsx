...
import {TrainingandCourseData } from "@/types/type";
...
import { addtrainingandcourse, deletetrainingandcourses, updatetrainingandcourse } from "@/store/slices/resumeSlice";
...
export default function TrainingAndCourses() {
 ...
 const TrainingAndCourses = useSelector((state:RootState)=> state.resumeBuilder.trainingandcourses)
  ...
  const EmptyForm:TrainingandCourseData = {
    id: "",
  ...
  const onSubmit: SubmitHandler<TrainingandCourseData> = (data) => {
    
    if (isEdit) {
      dispatch(updatetrainingandcourse(data));
       modifyResumeData(data,true);
    } else {
      const formData = {...data,id:crypto.randomUUID()}
      const templateData = {first:[],second:[],other:[]}
      dispatch(addtrainingandcourse(formData));
  ...
  const handleEdit = (id: string) => {
    const found =  TrainingAndCourses.find((d) => d.id === id);
    if (!found) return;
    reset(found);
    setIsEdit(true);
  };
  const onNext = async () => {
    ...
    if (EmptyDataChecked) {
      if ( TrainingAndCourses.length === 0) {
        const msg = "Please add at least one Training/Courses before proceeding.";
  ...
  return (
    <div>
      ...
      >
        ...
        <Inputs
          type="text"
          name="trainingandCoursename"
          label="Training/Courses Name"
          placeholder="Please Fill your Training/Courses Name"
          register={register}
          validation={{
            required: "Your Training/Courses Name is Required",
            pattern: {
              value: /^[A-Za-z\s]+$/,
              message: "Only Letter Are Allowed",
            },
          }}
          error={errors.trainingandCoursename}
        />
        <div className="relative">
          ...
          <textarea
            ...
            {...register("description",{required:"Please Tell Me About Your Training/Course"})}
          />
          {errors.description && (
        ...
        >
          {isEdit ? "Update " : "Add "}Training/Courses
        </button>
        {isEdit && (
          <button
            type="button"
      ...
      <AllSectionValue<TrainingandCourseData>
        title="Training/Courses"
        items={TrainingAndCourses}
        onEdit={(item) => handleEdit(item?.id)}
        onDelete={(item) =>{
          dispatch(deletetrainingandcourses(item?.id))
        ...
        renderItem={(item) => (
          <div>
            <h2>{item.trainingandCoursename}</h2>
